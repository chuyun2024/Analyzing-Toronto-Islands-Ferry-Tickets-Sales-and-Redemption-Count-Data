...
The remainder of this paper is structured as follows. @sec-data discusses the features of the raw and cleaned data, along with a summary of the cleaned data. @sec-result investigates the relationship between sales and redemption data using graphs, linear regression, and correlation analysis. It also explores the distribution of redemption counts by frequency and counts. @sec-discussion further elaborates on the implications and applications of the findings from @sec-result, addresses the limitations of the analysis, and suggests potential improvements.

# Data {#sec-data}

## Raw Data & Measurement

The data used in this paper is sourced from the Toronto Open Data portal and was imported using the opendatatoronto library (Gelfand, 2022). The raw dataset contains sales and redemption count data, recorded at 15-minute intervals, collected from both online and POS-kiosk sales. The dataset contains four variables: id, timestamp, sales count and redemption count. The id variable is a sequential number that records the order of the data entries. The dataset spans from July 11, 2023, to September 20, 2024, at the time of download [@toronto_ferry_tickets]. Similar datasets were not found online.
...
kable(summary_table,
  format = "latex", row.names = TRUE, col.names = c("Min", "Max", "Mean", "Standard Deviation"),
...
ggplot(data, aes(x = daily_sales_count, y = daily_redemption_count)) + # Set the data and aesthetic mappings; daily sales on the x-axis and daily redemption on the y-axis
  ...
  labs(
    title = "Scatter Plot of Sales vs Redemption", # Add a title to the plot
...
@fig-Daily_Redemption_Histogram illustrates that for most days, the redemption count remains below 10,000. However, there are some days where the redemption count peaks at 20,234.

For our resource allocation needs, utilizing monthly data would be sufficient to create an effective staffing schedule. Daily scheduling may not be as feasible or efficient, making a monthly approach more practical for our purposes. The monthly redemption distribution is analysed @sec-monred.
...
## Investigating the Monthly Redemption Data{sec-monred}
...
## Impact of Pricing Strategy on Revenue Optimization {#sec-first-point}
...
## Evaluation of Staffing and Departure Schedules in Relation to Visitor Demand
